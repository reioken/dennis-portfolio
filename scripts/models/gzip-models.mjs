...
//   node scripts/models/gzip-models.mjs [--only <name>[,<name>...]] [--alpha-quality 75] [--dry]
// <name> is relative to public/models without the extension (claw-v2, plush/whale-v1). Without --only, every
// model the hall sources reference as `.glb.gz` is rebuilt.
//
...
//      (webp-alpha.mjs; the colour bitstream stays byte-identical). Lossless alpha was ~80 % of each mask.
//   2. Meshopt level 'high' (filters: octahedral normals, quaternion/exponential animation channels) instead of
//      pack-models' 'medium'. Node, mesh, material and animation names must survive, as in pack-models.
...
// Measured 2026-09-25 on the 20 models of a first visit: 12.17 MB -> 8.17 MB (alpha q75: max error 2-3/255).
...
for (const name of names) {
  ...
  if (!PROTECTED.has(name)) {
    ...
    for (const texture of document.getRoot().listTextures()) {
      if (texture.getName() !== 'hero_mask' || texture.getMimeType() !== 'image/webp') continue;
