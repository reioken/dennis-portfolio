...
import {ROOM_POWER_MS, ROOM_POWER_SCALE, withRoomPower, roomPowerLevels, roomCircuitOffset, collectRoomPowerTargets} from '../../src/components/hall/roomPower.mjs';

// The assertions speak in authored cue time; playback is compressed by ROOM_POWER_SCALE.
const authored=roomPowerLevels;
const levelsAt=(ms,offset)=>authored(ms*ROOM_POWER_SCALE,offset);
...
test('dim ignition attempts fall back before a slower stable warm-up',()=>{
  assert.equal(levelsAt(0).circuit,0);
  for (const [peak,quiet] of [[780,1250],[1690,2250]]) {
    assert.ok(levelsAt(peak).circuit>0);
    assert.ok(levelsAt(peak).circuit<=.26);
    assert.equal(levelsAt(quiet).circuit,0);
  }
  assert.ok(levelsAt(3500).ambient<.1);
  assert.ok(levelsAt(5200).circuit<1);
...
test('circuits ignite at different times but all settle before completion',()=>{
  assert.notEqual(levelsAt(780).circuit,levelsAt(780,460).circuit);
...
test('screen power stays clean while fixtures strike and ambient light remains steady',()=>{
  assert.equal(levelsAt(780).screen,0);
  assert.equal(levelsAt(1690).screen,0);
  let previous=0;
...
test('shared baked lighting fades once and restores after a failed render',()=>{
 ...
 const factor=roomPowerLevels(2700).room;
 assert.throws(()=>withRoomPower(scene,2700,0,()=>{assert.ok(Math.abs(power.value-.8*factor)<1e-9);for(const m of materials)assert.ok(Math.abs(m.lightMapIntensity-2*factor)<1e-9);throw new Error('reflection render failed');}));
