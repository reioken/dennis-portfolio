...
| `repair-layout.mjs` | `node scripts/qa/hero/repair-layout.mjs OUT [BASE]` | Production panel screenshots at 320, 390, 844, 1024 and 1440 px; delayed mobile hydration exposes layout shifts. Records dimensions, shifts, overflow and console errors in `layout.json`. |
| `repair-flow.mjs` | `node scripts/qa/hero/repair-flow.mjs OUT [BASE]` | Delays wear maps and asserts their arrival before shader preparation; checks Essfreude hardware proxies, close-up, language/history navigation and phone gallery/rotation. |
| `mobile-hall.mjs` | `node scripts/qa/hero/mobile-hall.mjs OUT [BASE] [--before]` | Mobile hall at 320, 390, 768 and 844 px: 48 px targets, no overlaps/overflow, real touch swipe, directory and project-return flow, then desktop resize. `--before` records the old 390 px layout without assertions. |
