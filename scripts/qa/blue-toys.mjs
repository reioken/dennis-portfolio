...
for(const engine of (process.argv[2]?[process.argv[2]]:['chromium','firefox'])){
 ...
 try{for(const [width,reduce] of [[1440,false],[390,false],[1440,true]]){
  ...
  await page.goto(base);await page.waitForFunction(()=>window.__hall?.readyDone&&window.__hall.blue?.toys?.toys.length===2,null,{timeout:120000});await page.waitForTimeout(6500);await page.evaluate(()=>window.__hall.stop());
  ...
  const run=await page.evaluate(reduce=>{
   ...
   for(let i=0;i<60*30;i++){
    ...
    for(const t of b.toys.toys){maxSpeed=Math.max(maxSpeed,Math.hypot(t.velocity.x,t.velocity.z));if(!t.position.toArray().every(Number.isFinite)||t.position.y<t.radius-.001||t.position.x< -2.32||t.position.x>-.69||t.position.z<.29||t.position.z>.7)bad++;}
   }
   h.dirty=h.mirrorDirty=true;h.renderFrame();return{moods:[...moods],swats,maxSpeed,bad,position:b.body.position.toArray()};
  },reduce);
  assert.equal(run.bad,0);assert.ok(run.maxSpeed<=.86);if(!reduce)assert.ok(run.swats>0,'Blue reaches and bats the ball');else assert.deepEqual(run.position,initial);
  await mouse.focus();await page.keyboard.press('Enter');
  const mouseRun=await page.evaluate(reduce=>{const h=window.__hall,b=h.blue;let swats=0,previous='';for(let i=0;i<1500;i++){b.update(1/60,h.camera,true,reduce);if(b.mood==='swat'&&previous!=='swat')swats++;previous=b.mood;}return swats;},reduce);if(!reduce)assert.ok(mouseRun>0,'Blue also bats the mouse');
  ...
  assert.ok(rapid.max<=.86);assert.deepEqual(rapid.before,rapid.after);assert.equal(rapid.hidden,true);
