...
try{for(const width of [390,1366]){
 ...
 await page.goto('http://localhost:4322/contact/',{waitUntil:'networkidle'});await page.waitForFunction(()=>!document.documentElement.classList.contains('gl-pending'));
 await page.locator('.hall-panel__reading').click();assert.equal(await page.locator('html').evaluate(e=>e.classList.contains('is-reading')),true);
 ...
 await page.screenshot({path:`${out}/contact-${width}-validation.png`});await page.keyboard.press('Escape');assert.equal(await page.locator('html').evaluate(e=>e.classList.contains('is-reading')),false);
 await page.goto('http://localhost:4322/en/contact/',{waitUntil:'networkidle'});await page.waitForTimeout(800);assert.equal(await page.locator('html').getAttribute('lang'),'en');await page.locator('.hall-panel__reading').click();await page.getByRole('button',{name:'Send message',exact:true}).click();await page.getByRole('alert').waitFor();assert.equal(await page.locator('[aria-invalid=true]').count(),3);
 results.push({width,validation:true,mockSuccess:true,mockRateLimit:true,mockFailure:true,draftPreserved:true,readingMode:true,english:true,requests,errors});console.log(JSON.stringify(results.at(-1)));await page.close();
