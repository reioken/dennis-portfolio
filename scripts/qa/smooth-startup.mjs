...
try{for(const name of cases){
 ...
 await context.addInitScript(()=>{
  ...
  function sample(t){const el=document.querySelector('.hall__stage'),sc=window.__hall,loader=document.querySelector('.hall-startup'),style=el&&getComputedStyle(el),ls=loader&&getComputedStyle(loader);
   const frame={t,power:el?.dataset.power,phase:el?.dataset.startupPhase,opacity:style?.opacity,ready:sc?.readyDone,pending:sc?.pending,managed:sc?.managedLoading,loader:ls?.visibility==='visible'&&Number(ls?.opacity)>.01};
 ...
 try{
  ...
  if(name==='abort-gpu'){await page.waitForFunction(()=>document.querySelector('.hall__stage')?.dataset.startupPhase==='gpu',null,{timeout:25000});await page.locator('.site-nav__direct[href="/work/"]').click();await page.waitForURL('**/work/');await page.waitForTimeout(1200);}
  ...
  if(['no-webgl','failed-figure','timeout','abort-gpu'].includes(name)){
  ...
  assert.equal(report.frames.filter(f=>f.phase&&f.phase!=='ready'&&f.phase!=='failed'&&Number(f.opacity)>0).length,0,'no incomplete scene can be visible');
  assert.deepEqual(errors,[]);
  results.push({name,ok:true,dataset:report.dataset,frameCount:report.frames.length,visibleBeforeReady:report.frames.filter(f=>f.power==='loading'&&Number(f.opacity)>0).length,errors});
