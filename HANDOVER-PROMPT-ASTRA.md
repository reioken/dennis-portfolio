...
2. `HANDOVER-NEXT-AGENT.md` — Dennis's verdicts in his words, what was tried and rejected, the pipeline command table, open items. The status paragraphs at the top are in chronological order; the latest is the About typography/layout pass after Phase 21.
3. `HANDOVER-CLAUDE-FABLE.md` — site-wide state, deployment workflow, older decisions.
4. `docs/research/blue-cat.md` — the technical record of the cat, Phases 1–21. Phase 21 (the last section) is the current state.
...
- **Unjudged by Dennis:** the About consistency follow-up (`107f117` / `a1c29b93`: grouped numbers/labels, optical name alignment, smaller consistent body type and four adjacent heading icons), and Phase 21 (smooth petting angles, calmer reaction cadence, protected movement and corrected attention/inertialization history). Dennis reported rapid-input jank and the head flipping between sides. Assets remain v6d; no new explicit verdict on the Phase 20 sleep/coat or the performance pass as a whole. Wait for the live verdict.
...
- **File names:** `/models/*` and `/textures/*` are edge-cached for a day. Any content change under them needs a new file name. Blue's three files share the suffix `BLUE_ASSET_BUILD` in `src/components/hall/blueCat.ts` (currently `v6d`): bump it, pass it to the atlas script (4th argument) and use it in the pack output name. Delete the old files. This was learned the hard way on 2026-09-13.
...
- Blue: the revised curl's p99 stretch is 3.715 (folded-skin limits remain; around 11 mm underside penetration in the lab); a small hollow remains in the ring from straight above; the original atlas retains directional shading, and the eye/ear highlights still move at subpixel size; the front paws hover a few centimetres for a moment on the bed exit (no runtime foot IK); the turn at `BED_EXIT` before stepping in can take up to 2 s; contact-locked feet, KTX2 textures, foot slide on curved walking, the trot's look.
