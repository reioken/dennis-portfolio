...
2. `HANDOVER-NEXT-AGENT.md` — Dennis's verdicts in his words, what was tried and rejected, the pipeline command table, open items. The status paragraphs at the top are in chronological order; the last one is Phase 20.
3. `HANDOVER-CLAUDE-FABLE.md` — site-wide state, deployment workflow, older decisions.
4. `docs/research/blue-cat.md` — the technical record of the cat, Phases 1–20. Phase 20 (the last section) is the current state.
...
- **Unjudged by Dennis:** Phase 20 (Firefox coat-upload correction and cheek-down sleeping curl). Phase 19 was rejected with the blotchy-face screenshot and sleep feedback. The original atlas is byte-identical; do not paint over an upload defect. The performance pass remains unjudged as a whole. Wait for the live verdict.
