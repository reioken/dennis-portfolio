...
2. `HANDOVER-NEXT-AGENT.md` — Dennis's verdicts in his words, what was tried and rejected, the pipeline command table, open items. The status paragraphs at the top are in chronological order; the last one is Phase 19.
3. `HANDOVER-CLAUDE-FABLE.md` — site-wide state, deployment workflow, older decisions.
4. `docs/research/blue-cat.md` — the technical record of the cat, Phases 1–19. Phase 19 (the last section) is the current state.
...
- **Unjudged by Dennis:** Phase 19 (coat detail correction and responsive petting). He judged Phase 18 as blotchy/uneven and the touch too slow. The performance pass remains unjudged as a whole. Wait for his live verdict before another visual change.
...
- **File names:** `/models/*` and `/textures/*` are edge-cached for a day. Any content change under them needs a new file name. Blue's three files share the suffix `BLUE_ASSET_BUILD` in `src/components/hall/blueCat.ts` (currently `v6c`): bump it, pass it to the atlas script (4th argument) and use it in the pack output name. Delete the old files. This was learned the hard way on 2026-09-13.
...
- Blue: the curl's p99 stretch is 3.9 (a fold at the shoulder under the neck, invisible at hall size); a small hollow remains in the ring from straight above; the original atlas retains directional shading, and the eye/ear highlights still move at subpixel size; the front paws hover a few centimetres for a moment on the bed exit (no runtime foot IK); the turn at `BED_EXIT` before stepping in can take up to 2 s; contact-locked feet, native Firefox rerun, KTX2 textures, foot slide on curved walking, the trot's look.
