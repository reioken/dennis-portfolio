import {tier} from './core.js?v=2e123f57933a';
import {treasureValue,baseScore,settlement} from './rewards.js?v=2e123f57933a';
...
export function mountWelcome(){
 ...
 dialog.innerHTML=`<button class="welcome-close" aria-label="Close welcome">×</button><p class="welcome-kicker">Three digs. A fresh start every day.</p><h2 id="welcome-title" tabindex="-1" autofocus>Rarer answers.<br>Bigger hauls.</h2><p class="welcome-lead">Each answer must be rarer than your last.</p><section class="welcome-demo" aria-label="Practice example, separate from your game"><div class="welcome-demo-label"><span>Try it · Example finds</span><button class="welcome-replay">↻ Reset</button></div><ol class="welcome-chain"></ol><div class="welcome-world" aria-hidden="true"><div class="welcome-rail"></div><div class="welcome-mole"></div><div class="welcome-cart"><div class="welcome-gems"></div><img src="${art}v5/cart.png" alt=""></div><div class="welcome-spills"></div><span class="welcome-pop"></span></div><div class="welcome-haul"><strong class="welcome-points"></strong><span class="welcome-misses"></span></div><p class="welcome-feedback" role="status">Pick the rarer answer.</p><div class="welcome-answers"><button data-demo="rarer"></button><button data-demo="lower"><span>United States<small class="common">Common</small></span><strong>20<em>pts</em></strong></button></div><div class="welcome-risk"><span>Not rarer <b>−25%</b></span><span>× × × <b>Empty cart</b></span></div><button class="welcome-bank">↑ Climb out & save</button></section><button class="primary welcome-done">Got it. Let’s dig</button><p class="welcome-note">No timer. Unknown answers are free.</p>`;
 ...
 el('[data-demo="rarer"]').onclick=()=>{if(saved||mistakes===3||links===answers.length)return;links++;draw();effect('found','×'+links+'!');el('.welcome-feedback').textContent=links===3?'Three of a kind! +25% when you bank.':links===8?'Full spectrum too! Bank with ×1.5.':'Rarer! Your chain multiplier grows.';};
 el('[data-demo="lower"]').onclick=()=>{if(saved||mistakes===3)return;const before=el('.welcome-gems').childElementCount;const amount=mistakes===2?points():Math.ceil(points()*.25);lost+=amount;mistakes++;draw();effect('spilled','−'+amount,Math.max(1,before-el('.welcome-gems').childElementCount));el('.welcome-feedback').textContent=mistakes===3?'Third mistake. The cart is empty.':'That answer isn’t rarer. Some treasure spills.';};
