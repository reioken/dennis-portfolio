import {categories,score as oldScore} from './core.js?v=1479d059929f';
import {baseScore as score,settlement} from './rewards.js?v=1479d059929f';
import {edition,utcDay} from './edition.js?v=1479d059929f';
...
export function settleRound(state,runId,ci,chain,bust=false,risk={}){
 ...
 const reward=settlement(chain,risk.lost||0,bust);
 return {...state,rounds:[...state.rounds,{id,day:state.run.day,categoryId:categories[ci].id,chain:clone(chain),cargo:bust?[]:clone(risk.cargo||[]),points:reward.points,reward,mistakes:risk.mistakes||0,bust}]};
}
