import {mountWelcome} from './welcome.js?v=34cffeeb0050';
import {visualGuide} from './how-to.js?v=34cffeeb0050';
import {categories,tier,sum,score,assess,missed,shareText} from './core.js?v=34cffeeb0050';
import {MineScene} from './scene.js?v=34cffeeb0050';
import {BurrowView} from './burrow-view.js?v=34cffeeb0050';
import {findFor} from './feedback.js?v=34cffeeb0050';
import {mineLayout} from './geology.js?v=34cffeeb0050';
import * as legacyProgress from './progress.js?v=34cffeeb0050';
import * as dailyProgress from './daily-progress.js?v=34cffeeb0050';
import {edition,loadDaily,useDay,utcDay} from './edition.js?v=34cffeeb0050';
import {pointsLeft,takePenalty,dailyShare} from './risk.js?v=34cffeeb0050';
...
function settle(ending){
  stage=ending;results[ci]={name:categories[ci].label,chain:[...chain],bust:ending==='bust',...(wantsDaily?{points:haulPoints(),mistakes}:{})};
...
function showFind(answer){
  ...
  el.innerHTML='<img src="./assets/one-more-swing/'+scene.meta.images[f.asset]+'" alt=""><span><strong>'+f.tier+' find'+(f.level===3?'!':'')+'</strong><small>'+escape(answer[0])+' · '+answer[1]+' · '+f.name.toLowerCase()+'</small></span>';
}
...
function chainRows(){return '<ol class="chain">'+chain.map(a=>{const t=tier(a[1]);return '<li><strong>'+escape(a[0])+'</strong><small class="'+t.className+'">'+t.mark+' '+t.label+'</small><span class="rating '+t.className+'">'+a[1]+'</span></li>';}).join('')+'</ol>';}
function render(){
  ...
  if(stage==='home'){
    ...
    $('panel').innerHTML='<p class="eyebrow">All three digs complete</p><h1>That’s a good day’s digging.</h1><div class="result-score">'+total.toLocaleString('en-US')+'</div><p class="subtitle">Total points</p><pre class="share" id="share-text"></pre>';
    ...
    $('panel').innerHTML='<p class="eyebrow">Dig '+(ci+1)+' of 3</p><h1>'+categories[ci].name+'</h1><p class="subtitle">'+(stage==='play'?(chain.length?'Find something rarer than '+chain.at(-1)[1]+'.':'Start with any answer you know.'):stage==='banked'?(busy?'Bringing your haul home…':'Safely back at the surface.'):'That answer was too common.')+'</p>'+(chain.length?chainRows():'<p class="empty">Your first discovery starts the tunnel.</p>');
    if(stage==='play'){
      $('controls').innerHTML='<div class="scoreline"><strong>'+haulPoints()+' points</strong><small>'+sum(chain)+' × '+chain.length+' links</small></div><form class="answer-form" id="answer-form"><label class="sr" for="answer">'+categories[ci].name+'</label><input id="answer" autocomplete="off" autocapitalize="words" spellcheck="false" enterkeyhint="go" placeholder="'+(chain.length?'Rarer than '+chain.at(-1)[1]+'…':'Type an answer…')+'"><button class="submit" aria-label="Submit answer">→</button></form><p class="message" id="message">Unknown answers are free.</p><button class="primary" id="bank">Climb out · '+haulPoints()+'</button>';
      ...
      $('controls').innerHTML='<h2 class="ending-title">'+haulPoints()+' points banked</h2><p class="ending-sub">'+(exact?'You were 2 answers away from the top.':missing.length?'Rarer answers you missed.':'You reached the rarest answer in this category.')+'</p><ul class="missed">'+missing.map(a=>'<li><span>⬟ '+escape(a[0])+'</span><strong>'+a[1]+'</strong></li>').join('')+'</ul><button class="primary" id="next">'+(ci===2?'See today’s result':'Next category')+'</button>';
      $('next').onclick=next;
    }else{
      $('controls').innerHTML='<h2 class="ending-title">Chain ended. '+haulPoints()+' points kept.</h2><p class="ending-sub">'+escape(failed[0])+' '+failed[1]+' is not rarer than '+escape(chain.at(-1)[0])+' '+chain.at(-1)[1]+'.<br>'+sum(chain)+' × '+chain.length+' links</p><button class="primary" id="next">'+(ci===2?'See today’s result':'Next category')+'</button>';
...
async function submit(text){
  ...
  if(answer[1]>=80){scene.pose('celebrate');await scene.hold(610);}else await scene.hold(150);
  scene.pose('idle');hideFind();lock(false);status(answer[0]+' '+answer[1]+' · '+sum(chain)+' × '+chain.length+' = '+haulPoints()+' points. '+scene.haul.length+' '+(scene.haul.length===1?'find':'finds')+' in your cart.');announce(answer[0]+' accepted. '+haulPoints()+' points.');
}
async function bank(){
  ...
  lock(false);render();status(haulPoints()+' points brought home. '+homeProfile(progress).total+' total points. Missed answers do not count.');announce(haulPoints()+' points banked and brought home.');
}
async function mistake(answer){
  ...
  else {lock(false);render();$('message').textContent=answer[0]+' '+answer[1]+' isn’t rarer than '+chain.at(-1)[0]+' '+chain.at(-1)[1]+'. −'+penalty.amount+' points. '+(3-mistakes)+' mistakes left.';$('message').classList.add('error');announce($('message').textContent);}
}
async function bust(answer){
  ...
  await bringHome();lock(false);status('Chain ended. All '+chain.length+' finds and '+haulPoints()+' points brought home.');announce('Chain ended. '+haulPoints()+' points kept and brought home.');
}
...
if(wantsDaily){
  $('scope-close').onclick=()=>$('scope').close();
