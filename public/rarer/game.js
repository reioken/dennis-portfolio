import {treasureValue,treasureSum,baseScore,settlement,challenges} from './rewards.js?v=859643e99d5c';
import {bonusTrackers,rewardReceipt,bonusSummary} from './rewards-view.js?v=859643e99d5c';
import {mountWelcome} from './welcome.js?v=859643e99d5c';
import {visualGuide} from './how-to.js?v=859643e99d5c';
import {categories,tier,sum as oldSum,score as oldScore,assess,missed,shareText} from './core.js?v=859643e99d5c';
import {MineScene} from './scene.js?v=859643e99d5c';
import {BurrowView} from './burrow-view.js?v=859643e99d5c';
import {findFor} from './feedback.js?v=859643e99d5c';
import {mineLayout} from './geology.js?v=859643e99d5c';
import * as legacyProgress from './progress.js?v=859643e99d5c';
import * as dailyProgress from './daily-progress.js?v=859643e99d5c';
import {edition,loadDaily,useDay,utcDay} from './edition.js?v=859643e99d5c';
import {pointsLeft,takePenalty,dailyShare} from './risk.js?v=859643e99d5c';
...
function render(){
  ...
  if(scene.hillside){
    ...
    if(stage==='home')$('panel').innerHTML='<p class="eyebrow">Three new digs every day</p><h1>Go rarer. Go longer.</h1><p class="guide-intro">Rarer answers earn more. Beat your last find’s value.</p>'+visualGuide({risk:wantsDaily});
    if(stage==='play')$('controls').insertAdjacentHTML('afterbegin','<h2 class="answer-prompt">'+(chain.length?'Find '+escape(categories[ci].name.replace(/^A /,'a '))+' worth more than '+value(chain.at(-1)[1])+' points.':'Name '+escape(categories[ci].name.replace(/^A /,'a '))+'.')+'</h2>');
  ...
  if(wantsDaily){
    if(stage==='play'){
      ...
      $('message').textContent='Worth more = rarer. Small steps build bigger chains.';
...
async function mistake(answer){
  ...
  else {lock(false);render();$('message').textContent=answer[0]+' is worth '+value(answer[1])+' points, not more than '+chain.at(-1)[0]+' ('+value(chain.at(-1)[1])+'). −'+penalty.amount+' points. '+(3-mistakes)+' mistakes left.';$('message').classList.add('error');announce($('message').textContent);}
}
