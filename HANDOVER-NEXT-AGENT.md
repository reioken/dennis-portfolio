...
- Branch `feat/werkstatt`, head `d8028c0` (plus the handover commit on top), pushed. Production deployment `13214868` = build of `d8028c0`, verified by a headless browser load with console capture on the deployment URL and `www.dennisbf.design`, and by SHA-256 of `/models/blue-rigged-v6.glb` (1666108 bytes) and `/models/blue-v6-closed.webp` (248688 bytes) against the local `dist`.
- Blue v6: the v1 mesh (Meshy from Blue's photos) with its original atlas and painted amber eyes, 22 clips, Meshopt-packed GLB of 1.67 MB, two materials (`Blue coat v6`, `Blue amber eyes`), blink by texture swap plus the v1 `BlueBlink` squash, procedural bed, sleeping start, touch reactions, stepped turns, trot, inertialized transitions, tail/ear springs, sit in front of the cabinet, paws-only ledge hang. `blue-rigged-v5.glb`, `v4` and `v2` stay in `public/models/` for rollback (switch the path in `hallScene.ts`; the runtime routes by material name, `Blue coat vN` → `blue-vN-closed.webp`).
...
- v6 (Phase 16): the v1 mesh and atlas back, on the current clips and behaviours, blink by texture swap. Chosen by Dennis from a nine-model comparison sheet. **Not yet judged.**
...
| v6 export (the first cat) | `BLUE_ORIGINAL=1 blender --background --python scripts/models/blender/blue_animate.py` then `node scripts/models/blue-v6-closed-eyes.mjs` then `node scripts/models/blue-pack.mjs .source-assets/blue/v3/blue-rigged-v6.glb public/models/blue-rigged-v6.glb` | Uses `.source-assets/blue/v3/blue-rigged-master.blend` (v1 mesh + rig); keeps `Material_0` (→ `Blue coat v6`) and `Blue amber eyes`; the closed atlas is painted from the eye polygons |
...
Contact-locked feet, native Firefox rerun, KTX2 textures, foot slide on curved walking, the trot's look, the bed's size from the hall camera, `cat-animation.md` (Dennis's research report) is untracked in the repo root. v6-specific: the closed-eye patch is a fur-coloured fill with a dark seam (no real lid shape), the original atlas still has its baked highlight specks that earlier rounds removed (Dennis prefers the original; leave them unless he says otherwise), the closed atlas is 2048² (could go to 1024² if the budget bites), the perch clip still stands for its first half second. v5 leftovers: `blue_check.py` was never run on v5 or v6 through the current script.
