...
| Latest verified deployment | `https://80129228.dennis-portfolio-87g.pages.dev` (Blue v4 mesh, build of `f8bb85e`; headless 60 fps live recording clean, checksums match on both hosts) |
| Cloudflare Pages project | `dennis-portfolio` |
| Pages production branch argument | `main` |
| Local development | `http://localhost:4321/` |
| Latest delivered assets | `portrait-wide-v2-{1200,640}.webp`, `blue-rigged-v4.glb` (Meshy v4 mesh, 75k vertices, one PBR material, 22 clips, four morphs, Meshopt, 2,886,184 bytes) + `blue-v4-closed.webp` (closed-eye base colour for the blink); `blue-rigged-v2.glb` retained for rollback (switch the path in `hallScene.ts`) |
