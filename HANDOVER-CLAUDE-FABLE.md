...
Previous application releases: Phase 19 `199407c` / `ceb80036`; Phase 18 `07ba2b9` / `8973d2f7`; performance pass `940d46e` / `3f14487f`; Phase 17 `9ac596d` / `19e4d586`; selected v6 `d8028c0` / `13214868`. Older mesh/eye rounds remain historical below.
...
| Latest verified deployment | `https://107e5340.dennis-portfolio-87g.pages.dev` (Phase 20, build of `d85ce67`; both hosts checked in Firefox and Chromium, including direct About; older soft-transition issue remains) |
...
Latest successful application release sequence (commit `d85ce67`, deployment `107e5340`):

```powershell
$env:ASTRO_TELEMETRY_DISABLED="1"
npx astro build
npm run audit
npx wrangler pages deploy dist --project-name=dennis-portfolio --branch=main --commit-hash=d85ce67 --commit-dirty=true
```

Application commit precedes this build. Then verify both hosts, update both handovers, commit the documentation and push `feat/werkstatt`. Phase 20 checks: typecheck, 35 review tests, three Blue regressions plus `blue-petting.mjs` and Chromium/Firefox `blue-coat.mjs`, build and 52-page audit. Full SHA-256, CSP, browser errors/warnings/requests, context-loss provenance and images are in `.source-assets/blue/qa/sleep-coat-2026-09-13/live/`.
