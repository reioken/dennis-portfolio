...
Previous application releases: Phase 18 `07ba2b9` / `8973d2f7`; performance pass `940d46e` / `3f14487f`; Phase 17 `9ac596d` / `19e4d586`; selected v6 `d8028c0` / `13214868`. Older mesh/eye rounds remain historical below.
...
| Latest verified deployment | `https://ceb80036.dennis-portfolio-87g.pages.dev` (Phase 19, build of `199407c`; pages/assets/CSP/home-click checked on both hosts; pre-existing production About transition defect documented above) |
| Cloudflare Pages project | `dennis-portfolio` |
| Pages production branch argument | `main` |
| Local development | `http://localhost:4321/` |
| Latest delivered assets | `blue-rigged-v6c.glb` (1,206,368 bytes), `blue-v6c-mips.webp` (76,288), `blue-v6c-closed.webp` (2,036); same chosen v6 mesh/atlas source, 24 clips. Old v6b files removed. Portrait remains `portrait-wide-v2-{1200,640}.webp`. |
...
Latest successful application release sequence (commit `199407c`, deployment `ceb80036`):

```powershell
$env:ASTRO_TELEMETRY_DISABLED="1"
npx astro build
npm run audit
npx wrangler pages deploy dist --project-name=dennis-portfolio --branch=main --commit-hash=199407c --commit-dirty=true
```

Application commit precedes this build. Then verify both hosts, update both handovers, commit the documentation and push `feat/werkstatt`. Phase 19 checks: typecheck, 35 review tests, three Blue regressions plus `blue-petting.mjs`, build and 52-page audit. Full SHA-256, CSP, browser errors/requests and images are in `.source-assets/blue/qa/petting-2026-09-13/live/`.
