...
**Latest: Phase 23, 2026-09-14 — application `5eabead`, deployment `5528368d`.** Ball and mouse now support grab, lift and velocity-based throwing with mouse and touch. Blue continuously pursues toys, alternates forepaws, pounces and attempts reachable airborne catches. Actual paw contact drives impulses; misses remain misses. Head/eyes track the toy, ears perk and the tail sways. Bounded acceleration, steering and complete phrases protect repeated input. Dev probes also fixed arrival snapping, lost play requests on the return to bed, native touch-scroll cancellation and a reduced-motion target-position transition. Four added clips bring Blue to 29; all three filenames advance to v6f. GLB 1,289,900 bytes (+47,672); coat/eye pixels and companion textures stay identical, original v6 mesh and existing Meshy v1 toys stay. No new UI copy, generated assets or Meshy credits. Required Blue regressions, play (including real mobile touch/reduced motion), toys, petting, polish, both-browser coat checks, typecheck, 35 review tests, build and 53-page audit pass. Full record: `docs/research/blue-cat.md` Phase 23; evidence: `.source-assets/blue/play-2026-09-14/`. **Await Dennis's live verdict on throwing and play.** The Phase 22 sleep transition has no new verdict; rig limits and the previous About soft-transition issue remain separate.

**Phase 23 live verification:** [5528368d](https://5528368d.dennis-portfolio-87g.pages.dev) and [www.dennisbf.design](https://www.dennisbf.design), Chromium and Firefox: five routes return 200 with WASM CSP; all three v6f Blue files and both toy GLBs match dist by full SHA-256. Real desktop/mobile nudge and drag/throw, real Chromium mobile touch, keyboard mouse input and direct About with hidden toys pass. No page/console errors or failed requests. Known Firefox feature-probe context-release, upload and missing-extension warnings remain recorded. All 68 live captures inspected. Evidence: `.source-assets/blue/play-2026-09-14/live/`.

**Phase 22 shipped 2026-09-14** as `753581a` / deployment `e5f2a08d` (`docs/research/blue-cat.md`, Phase 22). Two generated reference images went through Meshy and Blender into a felt ball and mouse: 166,528 bytes total, 60 credits. They support bounded physics and real click/touch/keyboard nudges. Blue wakes, exits the bed, approaches and swats, finishing current movement before reacting to more input. Mobile positions remain visible; reduced motion allows small nudges without a chase. Dennis approved the two bilingual accessible labels. The creepy sleep transition was reproduced: interpolated torso points collapsed before bone-length repair; segment-direction interpolation now preserves lengths. Settle lowers/tucks before curling over 4.6 s; wake uncurls then rises over 2.8 s. Reused actions now restore their weight, fixing the pop to standing at a later sleep transition. All three Blue regressions, petting, polish, toys in Chromium/Firefox desktop/mobile/reduced motion, coat comparisons, typecheck, 35 review tests, build and 53-page audit pass. Blue assets are v6e with 25 clips. Original v6 mesh and coat/eye pixels stay. Sources and inspected evidence: `.source-assets/blue/toys-2026-09-14/`. **Dennis requested richer play and throwing in Phase 23; no new sleep verdict.** Prior About soft-transition issue remains separate.

**Live verification, 2026-09-14:** Deployment e5f2a08d and www.dennisbf.design pass in Chromium and Firefox: five routes return 200 with WASM CSP; all three v6e Blue files and both toy GLBs match dist by full SHA-256; desktop/mobile real ball input and keyboard mouse input work; direct About renders with toys hidden. No page/console errors or failed requests. Known Firefox feature-probe/upload/extension warnings remain recorded. All 36 live captures inspected. Evidence: `.source-assets/blue/toys-2026-09-14/live/`.

**About consistency follow-up shipped 2026-09-13** as `107f117` / deployment `a1c29b93` (`docs/research/about-typography-2026-09-13.md`, follow-up section). Dennis wanted numbers directly beside their labels, the visible first-name edge aligned with the text column, quieter consistent intro type, the work icon beside its heading, and icons on education/skills too. The dev probe found number-label gaps up to 94.25 px, an 8 px visible D-to-body edge difference at 1920 px, a 26.11 px intro beside 16 px body copy, a 564 px work-icon gap and three missing heading icons. Numbers now participate in the centered flex group (8 px gap; 6 px compact), with two rows only below 360 px panel width. A font-relative optical offset aligns the D; intro is 16 px / 1.65 like body copy, profile lead 18 px, desktop role 14 px. All four section headings use existing decorative 24 px icons directly beside the title (about, work, education, pen-nib). All section anchors were exercised at 320–1920 px, with no overflow. Three Blue regressions, typecheck, 35 review tests, build and 52-page audit pass. Both live hosts pass in Chromium and Firefox: five routes return 200 with WASM CSP, all three Blue assets plus font/CSS match dist SHA-256, desktop DE and mobile EN have centered navigation, adjacent icons and working anchors, with no page/console errors or failed requests. Live screenshots inspected; known Firefox warnings remain recorded. **Not yet judged by Dennis.** No new UI text; Blue v6d and the earlier production soft-transition hall issue are unchanged.

**About typography/layout shipped 2026-09-13** as `fbd99f1` / deployment `f388a7f4` (`docs/research/about-typography-2026-09-13.md`). Dennis asked for a more distinctive top, his complete name in uppercase, better use of width and centered section buttons. The existing name and section headings now use locally bundled Barlow Condensed 600 (22,308 bytes, OFL); body copy stays Outfit. The hero uses a wider copy area, a larger portrait nearer the right edge on wide panels, and equal-width contact actions. Section labels are centered independently of their numbers (up to 71.42 px left offset before, within 0.01 px after). On wide panels the profile and a compact 2×2 facts grid share a row; on phones the surname/role use the available width below the face. No new UI wording. All eight career entries, four education entries and 23 skill items remain. Dev measurements cover 320–2560 px without overflow; three Blue regressions, typecheck, 35 review tests, build and the 52-page audit pass. Both live hosts verified in Chromium and Firefox: five routes return 200 with WASM CSP; all three v6d files, the new font and About CSS match dist SHA-256; desktop DE and mobile EN render with loaded Barlow, all four section links work, no page/console errors or failed requests. Screenshots inspected. **Dennis judged this About pass in the consistency follow-up; Blue Phase 21 remains unjudged.** Blue remains unchanged at v6d; the earlier production About soft-transition hall issue remains separate.

**Start with `HANDOVER-NEXT-AGENT.md`.** **Phase 21 shipped 2026-09-13** as `da940da` / deployment `e354e82e` (`docs/research/blue-cat.md` Phase 21). Dennis could make Blue move fast and jerk by repeated input: "Wenn man den kopf streichelt dann flippt er hin und her ohne animation auch". The dev probe reproduced 23 whole-body switches from 24 alternating touches and a 23.38-degree one-frame head flip at 60 Hz. Raw side/region changes bypassed the smoothed touch envelope; reactions ran at 1.5x and tail flicks restarted at 1.6x. Inertialization also carried the attention overlay into a new clip, then applied it again. Pet angles now ease with a bounded angular speed; animation history is saved before attention; reactions run at 1x with a minimum phrase / completed arch, tail flicks finish and rest before another, and mapped walking/turning/sitting/perching/jumping cannot be cancelled by petting. Every touch still feeds the immediate soft response; no queued backlog. The same head test peaks at 2.72 degrees/frame, alternating touches cause one full-body switch. `blue-polish.mjs` adds real-pointer guards, with all 96 frames inspected. Three Blue regressions, petting, typecheck, 35 review tests, build and 52-page audit pass. Both hosts verified in Firefox and Chromium: routes 200, all three v6d files and the actual Stage3D runtime match dist SHA-256, WASM CSP present, real rapid head clicks, no page/console errors or failed requests. **Dennis has not judged Phase 21 live.** Original v6d model/atlas/clip files are unchanged; the Phase 20 sleep/coat work has no new explicit visual verdict. The older production About soft-transition issue remains separate.
...
Previous application releases: Phase 20 `d85ce67` / `107e5340`; Phase 19 `199407c` / `ceb80036`; Phase 18 `07ba2b9` / `8973d2f7`; performance pass `940d46e` / `3f14487f`; Phase 17 `9ac596d` / `19e4d586`; selected v6 `d8028c0` / `13214868`. Older mesh/eye rounds remain historical below.
...
| Latest verified deployment | `https://5528368d.dennis-portfolio-87g.pages.dev` (Phase 23, build of `5eabead`; both hosts / Firefox and Chromium; older soft-transition issue remains) |
| Cloudflare Pages project | `dennis-portfolio` |
| Pages production branch argument | `main` |
| Local development | `http://localhost:4321/` |
| Latest delivered assets | `blue-rigged-v6f.glb` (1,289,900 bytes), `blue-v6f-mips.webp` (76,288), `blue-v6f-closed.webp` (2,036); original v6 mesh/atlas, 29 clips. Old v6e filenames removed. Ball/mouse remain v1, 166,528 bytes together. Portrait remains `portrait-wide-v2-{1200,640}.webp`. |
...
Latest successful application release sequence (commit `da940da`, deployment `e354e82e`):

```powershell
$env:ASTRO_TELEMETRY_DISABLED="1"
npx astro build
npm run audit
npx wrangler pages deploy dist --project-name=dennis-portfolio --branch=main --commit-hash=da940da --commit-dirty=true
```

Application commit precedes this build. Then verify both hosts, update both handovers, commit the documentation and push `feat/werkstatt`. Phase 21 checks: typecheck, 35 review tests, three Blue regressions plus `blue-petting.mjs` and `blue-polish.mjs`, build and 52-page audit. Both-browser live hashes/CSP/errors/warnings and head-petting images are in `.source-assets/blue/qa/polish-2026-09-13/live/`. Model and textures are unchanged v6d.
