...
Latest application commit: **`2b20ea9`** (Blue v3: root-motion jumps, on top of `da3f3b5` turning with steps and `0f64940` corrective/ledge fix), pushed to `origin/feat/werkstatt` and **published on 2026-09-12 in the evening** (deployment `46222b52`, build of `2b20ea9`). The agent's permission classifier blocked the upload twice; Dennis then instructed it explicitly and the upload ran through PowerShell. Verified after upload on both the deployment URL and `www.dennisbf.design`: `/`, `/about/`, `/en/about/` return 200 and `/models/blue-rigged-v2.glb` (2,292,652 bytes) has the same SHA-256 as the local `dist`. HTTP/asset verification, not a production browser sweep.
...
| Latest verified deployment | `https://46222b52.dennis-portfolio-87g.pages.dev` (Blue v3, build of `2b20ea9`) |
